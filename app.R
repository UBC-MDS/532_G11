...
# Create side panels to avoid duplication
sidepanels_rt <- sidebarPanel(pickerInput("genre_rt", "Genre(s):", choices = sort(unique(str_replace_all(string=unlist(strsplit(movies$Genre,',')), pattern=" ", repl=""))),
                                       options = list(`actions-box` = TRUE),multiple = T, selected = 'Drama'), 
                           # Select single star
                           pickerInput("star_rt", "Star(s):", choices = sort(unique(unlist(select(movies,Star1:Star4) %>% as.list()))),
                                       options = list(`actions-box` = TRUE, `live-search` = TRUE),multiple = T, selected = 'Morgan Freeman'),
                           # select min movie rating
